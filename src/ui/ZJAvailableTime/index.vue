<template>
  <div class="availableDaysBox" @click="clickSelect">
    <div class="available-body">
      <div class="uu">
        <div class="available-body-box" v-for="m in title" :key="m">{{ m }}</div>
      </div>
      <div>
        <div :class="{ 'available-body-box': true, 'available-body-box-selected': m == 'true' }" v-for="(m, index) in mon"
          :key="index">
          <div v-if="index < 1">{{ m }}</div>
        </div>
      </div>
      <div>
        <div :class="{ 'available-body-box': true, 'available-body-box-selected': m == 'true' }" v-for="(m, index) in thu"
          :key="index">
          <div v-if="index < 1">{{ m }}</div>
        </div>
      </div>
      <div>
        <div :class="{ 'available-body-box': true, 'available-body-box-selected': m == 'true' }" v-for="(m, index) in wed"
          :key="index">
          <div v-if="index < 1">{{ m }}</div>
        </div>
      </div>
      <div>
        <div :class="{ 'available-body-box': true, 'available-body-box-selected': m == 'true' }" v-for="(m, index) in tue"
          :key="index">
          <div v-if="index < 1">{{ m }}</div>
        </div>
      </div>
      <div>
        <div :class="{ 'available-body-box': true, 'available-body-box-selected': m == 'true' }" v-for="(m, index) in fri"
          :key="index">
          <div v-if="index < 1">{{ m }}</div>
        </div>
      </div>
      <div>
        <div :class="{ 'available-body-box': true, 'available-body-box-selected': m == 'true' }" v-for="(m, index) in sat"
          :key="index">
          <div v-if="index < 1">{{ m }}</div>
        </div>
      </div>
      <div class="uuu">
        <div :class="{ 'available-body-box': true, 'available-body-box-selected': m == 'true' }" v-for="(m, index) in sun"
          :key="index">
          <div v-if="index < 1">{{ m }}</div>
        </div>
      </div>
  ...
  <availablDaysSelect class="modal" v-show="showSelectAvailableDays" @close="close()" @showSelectData="acceptShowData"
    @showSelectDataAll="acceptShowDataAll" :readAvailableData="props.readAvailableData" :type="props.type">
  </availablDaysSelect>
</template>
